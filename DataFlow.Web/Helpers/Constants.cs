namespace DataFlow.Web.Helpers
{
    public struct Constants
    {
        public const string API_SERVER_URL = "API_SERVER_URL";
        public const string API_SERVER_KEY = "API_SERVER_KEY";
        public const string API_SERVER_SECRET = "API_SERVER_SECRET";
        public const string DEFAULTS_TERM_MONTH = "DEFAULTS_TERM_MONTH";
        public const string DEFAULTS_TERM_YEAR = "DEFAULTS_TERM_YEAR";
        public const string INSTANCE_COMPANY_NAME = "INSTANCE_COMPANY_NAME";
        public const string INSTANCE_COMPANY_LOGO = "INSTANCE_COMPANY_LOGO";
        public const string INSTANCE_COMPANY_URL = "INSTANCE_COMPANY_URL";
        public const string INSTANCE_EDU_USE_TEXT = "INSTANCE_EDU_USE_TEXT";
        public const string ConfigurationCacheKey = "ConfigurationCacheKey";
        public const string AppSettingEncryptionKey = "EncryptionKey";
    }
}